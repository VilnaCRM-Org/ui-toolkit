...
| Story 3.7 (`3-7-board-follow-up-controls.md`)                            | `Complete` — `art §1 Changed files` inventories all 103 changed paths, grouped by module, showcase, tests, manifest and registers | `Complete` — `reg` "Story 3.7 — Board Follow-Up Controls" (eight new rows, source `new`) and the extended Epic 1 `UiButton` row; `led DEV-51`-`DEV-60` record every deviation                                                           | `Complete` — eight `tests/unit/ui-<kebab>.test.tsx` suites (`ui-background-picker`, `ui-option-card`, `ui-chevron-button`, `ui-add-button`, `ui-clear-button`, `ui-copy-field`, `ui-social-icon-button`, `ui-segmented-control`) plus the danger-variant block appended to `tests/unit/ui-button.test.tsx` | `Complete` — eight `*.stories.tsx` files plus a `Danger` story in `button.stories.tsx` (title `'UiComponents/Ui<Name>'`, `tags: ['autodocs']`); `tests/visual/stories.json` regenerated from the live index, 120 → 134 entries. Pixel baselines for the 14 new story ids remain an open follow-up, recorded in the story's own DoD table | `Complete` — `src/components/index.ts` carries eight new default+type export pairs (`UiButton` gains no export, only a prop value); `chk` Board A/B rows above cite the barrel                                                                                                                                               | `Complete` — `art §Design sources — Figma node map` carries per-state node ids for all nine components (background-picker, button-danger, option-card, chevron-button, add-button, clear-button, copy-field, social-icon-button, segmented-control) | `Complete`           | Open — PR [#150](https://github.com/VilnaCRM-Org/ui-toolkit/pull/150), draft, base `feat/issue-33-export-contract`; issue [#149](https://github.com/VilnaCRM-Org/ui-toolkit/issues/149) OPEN |
