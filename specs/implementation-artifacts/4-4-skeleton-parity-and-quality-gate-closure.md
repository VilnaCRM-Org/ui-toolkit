...
| `ui-skeleton-block`        | `UiSkeletonBlock`       | `skeleton-block.stories.tsx`        | 3         | `tests/unit/ui-skeletons.test.tsx` (shared)    |
| `ui-skeleton-button`       | `UiSkeletonButton`      | `skeleton-button.stories.tsx`       | 1         | `tests/unit/ui-skeletons.test.tsx` (shared)    |
...
| `ui-skeleton-image`        | `UiSkeletonImage`       | `skeleton-image.stories.tsx`        | 3         | `tests/unit/ui-skeletons.test.tsx` (shared)    |
| `ui-skeleton-input`        | `UiSkeletonInput`       | `skeleton-input.stories.tsx`        | 2         | `tests/unit/ui-skeletons.test.tsx` (shared)    |
...
| `ui-skeleton-text`         | `UiSkeletonText`        | `skeleton-text.stories.tsx`         | 4         | `tests/unit/ui-skeletons.test.tsx` (shared)    |
...
gains an `Epic 4` entry. The guard accepts only a behaviour suite whose filename is keyed to the
module, so the entry gates the seven modules that have one — `auth-skeleton`,
`ui-skeleton-control-text`, `ui-skeleton-list`, `ui-skeleton-menu`, `ui-skeleton-tab-bar`,
`ui-skeleton-table` and `ui-skeleton-widget`. The five primitives (`UiSkeletonBlock`,
`UiSkeletonButton`, `UiSkeletonImage`, `UiSkeletonInput`, `UiSkeletonText`) are covered by the
shared primitive suite instead; splitting it into per-module suites so the guard can gate them
too is an open follow-up, not a waiver of the guard's rule.
...
table. The surfaces are machine-held by the closure guard for the seven dedicated-suite modules,
and by `tests/unit/components-index.test.ts` plus the shared primitive suite for the other five.
