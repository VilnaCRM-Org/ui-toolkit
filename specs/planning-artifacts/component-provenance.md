...
| ------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `UiButton` (`src/components/ui-button`)     | `crm`  | Core control already present from the canonical toolkit; behavior follows `crm` button semantics. | States `rest/hover/active/disabled` styled from `crm`-canonical tokens (`ui-color-theme`). **Deviation:** `error` is N/A for the button role (per `prd.md:78`). `socialButton` variant per-state visual coverage deferred (non-blocking) — see Story 1.2 artifact.          |
...
| `UiLink` (`src/components/ui-link`)         | `crm`  | Canonical link behavior.                                                                          | States `rest/hover/active` styled from `crm` tokens. **Deviation:** `disabled`/`error` are N/A for a link and are declared contract exceptions in `ui-link/types.ts`.                                                                                                       |
