...
// a dev-only teaching warning and never a gate. It is the one warning that reads
// the DOM rather than the props, so it runs from a mount effect against the real
// node instead of through `useDevWarning`; `devWarn` still keeps it silent in
// production. The deps are both stable, so it fires once.
...
interface CardNodeAssignment {
  ...
  /** The component's own handle, read only by the radiogroup-ancestor check. */
  own: React.RefObject<HTMLButtonElement | null>;
  node: HTMLButtonElement | null;
}

// The forwarded ref lands on the card button itself — there is no positioning
// wrapper, because nothing in this card floats. React accepts a callback ref or a
// ref object, so both shapes are assigned explicitly.
 ...
 * private handle on the node, which the radiogroup-ancestor check reads on mount.
 * The component itself never calls `.focus()` on it — focus behaviour is entirely
 * the platform's; the ref exists purely as the consumer's focus-return API.
 */
