...
describe('pinCellSx — the 64x86 master (pure, mutation-killing)', () => {
  ...
  it('keeps the native primary caret and paints no decorative bar', () => {
    // Owner ruling (2026-08-26): the caret stays the browser's own, tinted
    // primary; nothing is background-painted or pseudo-element-drawn.
  ...
  it('paints the Figma active shadow on focus, ungated for disabled cells', () => {
    expect(PIN_FOCUS_SHADOW).toBe('0 7px 12px rgba(76, 90, 126, 0.15)');
    // Ungated on purpose: with the dark ring removed (owner ruling 2026-08-26),
    // the shadow is the focused DISABLED cell's only remaining cue.
    expect(ruleAt(ACTIVE_KEY)).toEqual({ boxShadow: PIN_FOCUS_SHADOW });
  });

  it('carries no dark ring: focus-visible keeps only the Figma shadow (owner ruling)', () => {
    // The Amendment-A1 inset ring was REMOVED by owner ruling (2026-08-26,
    // recorded at gate review) — the focused cell shows exactly the Figma
    // chrome. The two-selector list survives so `outline: none` still ties the
    // hover specificity and the forced-colors block below can win over it.
